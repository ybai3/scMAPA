...
def parse_cfgfile(cfg_file):
    ...
    if Group1_Tophat_aligned_file=='':
        print("No Tophat aligned BAM file for group 1!", sys.stderr)
        exit(1)
    if Group2_Tophat_aligned_file=='':
        print("No Tophat aligned BAM file for group 2!", sys.stderr)
        exit(1)
    if output_directory=='':
        print("No output directory!", sys.stderr)
        exit(1)
    if Annotated_3UTR_file=='':
        print("No annotated 3' UTR file!", sys.stderr)
        exit(1)
    if Output_result_file=='':
        print("No result file name!", sys.stderr)
        exit(1)
...
def De_Novo_3UTR_Identification_Loading_Target_Wig_for_TCGA_Multiple_Samples_Main(argv=None):
    ...
    if len(sys.argv) == 1:
        ...
        exit(1)
    cfg_file = sys.argv[1]
    print("[%s] Start Analysis ..." % time_now(), sys.stderr)
    ...
    ##Debug
    print("[%s] Loading coverage ..." % time_now(), sys.stderr)
    ...
    print("[%s] Loading coverage finished ..." % time_now(), sys.stderr)
    ...
    print("[%s] Filtering the result ..." % time_now(), sys.stderr)
    ...
    print("[%s] Finished!" % time_now(), sys.stderr)
